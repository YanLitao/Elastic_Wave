...
crack_start_li = linspace(50, 250, 9);
crack_end_li = linspace(65, 265, 9);
offset_li = linspace(src_nz+40, 270, 9);
crack_plane = [];
crack_li = [];
% 9 cracks
for n=1:length(crack_start_li)
    crack_plane = [crack_plane, linspace(crack_start_li(n), crack_end_li(n), crack_end_li(n)-crack_start_li(n)+1)];
    crack_li = [crack_li; linspace(crack_start_li(n), crack_end_li(n), crack_end_li(n)-crack_start_li(n)+1)];
end
...
for n=1:nt
    ...
    % 2 cracks
    for m=1:length(crack_start_li)
        Txz(crack_start_li(m): crack_end_li(m), offset_li(m)) = zeros(crack_end_li(m)-crack_start_li(m)+1,1);
    end
    ...
    if (mod(n,5)==0)
        ...
        % two cracks
        for nc=1:length(crack_start_li)
            plot(x2d(crack_start_li(nc):crack_end_li(nc),offset_li(nc)),z2d(crack_start_li(nc):crack_end_li(nc),offset_li(nc)),'b-','LineWidth',2);
        end 
        %source position
